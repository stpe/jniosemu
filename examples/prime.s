...
###################################
# HEXASC
# In: r4, Out: r2, Destroys: -
###################################

hexasc:	PUSH	r15
	andi	r2, r4, 0x0F		# mask 4 LSBits of return register
	cmplei	r15, r2, 0x09		# r16 := 1 if 0..9
	beq	r15, r0, AtoF		# branch if 10-15
num:	addi	r2, r2, 0x30		#
	br	home
...
maa:    mov     r4, r16         # r16 <- NextPrime(r16)
	...
	mov     r4, r17         # Print:
	call	hex2dec
	movia	r4, text2	# Print: " is: "
	call	prtext
	mov     r4, r16         # Print:
	call	hex2dec
	movia	r5, uart0	# Print: "\n"
	movi	r4, 0xa		#
	call	nr_uart_txchar	#
	movia	r5, uart0	#
	movi	r4, 0xd		#
...
hex2dec:PUSH	r31		# save
	...
	mov	r8, r4		# r8 = CURRENT VALUE

	mov	r12, r0		# Ide: r12 = NUMBER OF DIGITS in r8


	movi	r4, 1		# Ide: push 1, 10, 100, ..., 10^N
push10:	push	r4		# push next 10-potens
	addi	r12, r12, 1	# i.e. there is a yet another number
	call	mul10		# next 10-potens
	mov	r4, r2		# to r4
	ble	r4, r8, push10	# larger than current?

pop10:	pop	r10		# pop largest 10-potens
	mov	r11, r0		# Ide: calc digit in corresponding position
sub10:	blt	r8, r10, done10	# while (digit>0)
	addi	r11, r11, 1	#   cnt++
	sub	r8, r8, r10	#   digit--
	br	sub10
done10:	putreg	r11		# cout << digit
	subi	r12, r12, 1	# move to next digit (lower 10-potens)
	bgt	r12, r0, pop10
...
###################################
# NextPrime
# In: r4, Out: r2, Destroys: r8, r9, r10, r11
###################################
...
Pr1:	addi	r8, r8, 1	# add 1
	andi	r9, r8, 1	# mask odd bit
	bne	r9, r0, fyy
	addi	r8, r8, 1	# add one if necessary

fyy:	mov	r9, r8		# copy to r9
	srli	r9, r9, 1	# divide with 2, MAX
	movi	r10, 2		# DIV gives start value

Pr2:	mov	r11, r8		# Rem := NUMBER
Pr3:	sub	r11, r11, r10
