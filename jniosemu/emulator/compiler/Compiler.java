...
public class Compiler {
   ...
   */
  public static long parseValue(
      String aValue, Hashtable<String, Integer> aLabels, int aAddr, int aDivider)
      throws InstructionException {
    if (aValue.length() == 0) return 0;
  ...
  private void parseLine(String aLine, boolean aFirst, int aLineNumber) throws CompilerException {
    ...
    // Find the important part of the line. That will say remove commants and whitespace in the
    // beginning and end
          ...
          if (mInstruction.matches()) {
            ...
            if (mInstruction.group(1) != null) {
              ...
              if (name.equals("equ")) {
                ...
                if (mMacro.matches()) {
                  this.lastMacro =
                      this.macros.put(mMacro.group(1), mMacro.group(2), null, aLineNumber, null);
                } else {
                ...
                try {
                  ...
                  Variable.TYPE type;
                  if (name.equals("word")) type = Variable.TYPE.WORD;
                  else if (name.equals("hword")) type = Variable.TYPE.HWORD;
                  else if (name.equals("byte")) type = Variable.TYPE.BYTE;
                  else if (name.equals("ascii")) type = Variable.TYPE.ASCII;
                  else type = Variable.TYPE.ASCIZ;
                ...
                if (mFile.matches()) {
                  ...
                  try {
                    ...
                    if ((content = Editor.read(path)) != null) {
                      ...
                      for (int i = 0; i < aLineNumber; i++) tmpLines[i] = new String(this.lines[i]);
                      ...
                      for (int i = 0; i < this.lines.length - aLineNumber; i++)
                        tmpLines[aLineNumber + 2 + lines.length + i] =
                            new String(this.lines[aLineNumber + i]);
                      this.lines = tmpLines;
                    }
                  } catch (IOException e) {
              ...
              if (this.macros.exists(ins)) {
                ...
                if (this.lastMacro != null) {
                  ...
                  try {
                    CompilerInstruction cins =
                        InstructionManager.get(ins, mInstruction.group(4), aLineNumber);
  ...
  public Program link() throws CompilerException {
    int size = 0;
    for (Variable variable : this.variables) size += variable.getLength();

    int diff = size % 4;
    if (diff > 0) size += 4 - diff;
    ...
    try {
      return new Program(
          this.lines, this.instructions, this.variables, binaryProgram, binaryVariables, pc);
    } catch (InstructionException e) {
      // Could not happen
      throw new CompilerException();
    }
  ...
  /**
   * Returns input with all multiline comments removed. Uses regex from
