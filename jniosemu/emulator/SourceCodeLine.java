...
public class SourceCodeLine {
  ...
  public static enum SIBLINGSTATUS {
    FIRST,
    LAST,
    ME,
    MIDDLE,
  ...
  public static String replaceTabWithSpaces(String str, int spacesPerTab) {
    ...
    for (int i = 0; i < str.length(); i++) {
      if (str.charAt(i) == '\t') {
        ...
        for (int j = 0; j < spaces; j++) ret = ret.concat(" ");
        addedChars += spaces - 1;
      } else {
  ...
  public SIBLINGSTATUS isSibling(int lineNumber) {
    if (this.lineNumber == lineNumber) return SIBLINGSTATUS.ME;
    ...
    if (parentStart <= lineNumber && parentEnd >= lineNumber) {
      if (parentStart == this.lineNumber) return SIBLINGSTATUS.FIRST;
      if (parentEnd == this.lineNumber) return SIBLINGSTATUS.LAST;
  ...
  public String getSourceCodeLine() {
    if (this.sourceCodeLine == null) return null;
  ...
  public String toString() {
    String ret = "";
    if (this.getSourceCodeLine() != null) ret += this.getSourceCodeLine() + "\n";
    if (this.getOpCode() != null) ret += this.getOpCode() + " " + this.getInstruction();
    return ret;
  }

  /**
   * Toggle breakpoint. If DISABLED nothing happens. Else TRUE -> FALSE and FALSE -> TRUE.
   *
