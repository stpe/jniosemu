...
public class Utilities {
  ...
  public static byte vectorToByte(Vector<Boolean> vector) {
    ...
    int i = 0;
    for (Boolean value : vector) {
      if (value) ret |= (byte) Math.pow(2, i);
      i++;
    }
    return ret;
  }
  ...
  public static byte[] shortToByteArray(short value) {
    ...
    for (int i = 0; i < 2; i++) ret[i] = (byte) (value >>> (8 * i) & 0xFF);
  ...
  public static byte[] intToByteArray(int value) {
    ...
    for (int i = 0; i < 4; i++) ret[i] = (byte) (value >>> (8 * i) & 0xFF);
  ...
  public static byte[] longToByteArray(long value) {
    ...
    for (int i = 0; i < 8; i++) ret[i] = (byte) (value >>> (8 * i) & 0xFF);
  ...
  public static long byteArrayToLong(byte[] value) {
    ...
    for (int i = 0; i < 8; i++) ret |= ((value[i] & 0xFFL) << (8 * i));
  ...
  public static Vector<Boolean> intToVector(int value, int size) {
    ...
    for (int i = 0; i < size; i++) ret.add(i, (value & (int) Math.pow(2, i)) > 0);
  ...
  public static String byteArrayToBitString(byte[] value) {
    ...
    for (int i = value.length - 1; i >= 0; i--) {
      for (int j = 7; j >= 0; j--) {
        if ((value[i] & (int) Math.pow(2, j)) > 0) ret = ret.concat("1");
        else ret = ret.concat("0");
      }
  ...
  /**
   * Converts byte array to string where non-printable characters are displayed as a period (".").
   *
