...
/**
 * The GUI Manager creates all components of the GUI and dictates their layout. It also manages
 * general actions of the GUI not related to specific components.
 */
public class GUIManager implements EventObserver {
  ...
  private void setup(JPanel mainPanel) {
    ...
    editorSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, editor, editorMessages);

    // remove splitpane keyboard bindings (it eats F8)
    SwingUtilities.replaceUIInputMap(
        editorSplitPane, JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT, null);
    ...
    // split pane between editor and editor messages
    emulatorSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, emulator, emulatorMessages);

    // remove splitpane keyboard bindings (it eats F8)
    SwingUtilities.replaceUIInputMap(
        emulatorSplitPane, JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT, null);
    ...
    JSplitPane registerSplitPane =
        new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, emulatorLeftPanel, emulatorSplitPane);
    emulatorPanel.add(registerSplitPane, BorderLayout.CENTER);

    // remove splitpane keyboard bindings (it eats F8)
    SwingUtilities.replaceUIInputMap(
        registerSplitPane, JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT, null);
  ...
  /**
   * Display information about exception and output stacktrace to stdout.
   *
   * @calledby update()
  ...
  /**
   * Show Variables View window positioned in center of main window.
   *
  ...
  /**
   * Show Memory View window positioned in center of main window.
   *
   * @calls GUIMemoryView
   * @calledby update()
   */
  ...
  /**
   * Show Serial Console window positioned in center of main window.
   *
   * @calls GUIUART
   * @calledby update()
   */
  ...
  /**
   * Show Console window positioned in center of main window.
   *
   * @calls GUIConsole
   * @calledby update()
   */
   ...
   */
  private void toggleMessagesSplitpane(
      boolean makeVisible, JComponent msgView, JSplitPane splitpane) {
    // do nothing if state already is set
    if (makeVisible == msgView.isVisible()) return;
