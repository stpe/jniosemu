...
public class GUIIOButtons extends JPanel implements EventObserver {
    ...
    for (int i = 0; i < buttons.length; i++) {
      ...
      buttons[i].addMouseListener(
          new MouseAdapter() {
             ...
             *
             * @checks Only send event if it is the left mouse button that is pressed.
            ...
            public void mousePressed(MouseEvent e) {
              if (e.getButton() == MouseEvent.BUTTON1) {
                eventManager.sendEvent(
                    EventManager.EVENT.BUTTON_PRESS, Integer.valueOf(buttonIndex));
              }
            }
             ...
             *
             * @checks Only send event if it is the left mouse button that is released.
             * @calls EventManager.sendEvent()
            ...
            public void mouseReleased(MouseEvent e) {
              if (e.getButton() == MouseEvent.BUTTON1) {
                eventManager.sendEvent(
                    EventManager.EVENT.BUTTON_RELEASE, Integer.valueOf(buttonIndex));
              }
            }

            /**
             * Send event when right mouse button is clicked. Used to toggle state for button so
             * user don't have to manually press the button while stepping through code.
             *
             * @checks Only send event if it is the right mouse button that is clicked.
             * @calls EventManager.sendEvent()
            ...
            public void mouseClicked(MouseEvent e) {
              if (e.getButton() == MouseEvent.BUTTON3) {
                eventManager.sendEvent(
                    EventManager.EVENT.BUTTON_TOGGLE, Integer.valueOf(buttonIndex));
              }
            }
          });
   ...
   * Set state of button.
   *
   * @checks Verify that buttonIndex is a valid index. If not, print warning and return.
  ...
  public void setButton(int buttonIndex, boolean state) {
    ...
    if (state) buttons[buttonIndex].setIcon(icons[1]);
    else buttons[buttonIndex].setIcon(icons[0]);
  }
