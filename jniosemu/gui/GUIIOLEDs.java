...
public class GUIIOLEDs extends JPanel implements EventObserver {
   ...
   * Set state of led.
   *
   * @checks Verify that ledIndex is a valid index. If not, print warning and return.
   * @calledby updateLeds
  ...
  public void setLed(int ledIndex, boolean state) {
    ...
    if (state) leds[ledIndex].setIcon(icons[1]);
    else leds[ledIndex].setIcon(icons[0]);
  }
