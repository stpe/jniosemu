...
public class GUIEditor extends JPanel
    ...
    textArea =
        new JTextArea() {
          ...
          protected void processEvent(AWTEvent e) {
            ...
            if (!isNewline) super.processEvent(e);
          }
        };
  ...
  /**
   * Returns true if current document has changed since last save.
   *
  ...
  /**
   * Is called when text is changed in editor. Used to keep track of if a document is modified since
   * last save or not.
   *
   * @calledby insertUpdate(), removeUpdate(), changedUpdate(), newDocument(), openDocument(),
   *     saveDocument()
   * @calls EventManager.sendEvent()
   * @param textHasChanged set value if text has changed
   * @param forceTitleUpdated forces a window title update even if textHasChanged state is same
   */
  private void textChanged(boolean textHasChanged, boolean forceTitleUpdate) {
    if (this.textHasChanged != textHasChanged || forceTitleUpdate) {
      ...
      eventManager.sendEvent(
          EventManager.EVENT.APPLICATION_TITLE_CHANGE, getDocumentTitleSuffixed());
    }
  }
  ...
  /**
   * Exists due to DocumentListener interface. Detects when something is updated in the editor.
   *
   * @calls textChanged()
   */
  ...
  /**
   * Exists due to DocumentListener interface. Detects when something is inserted in the editor.
   *
   * @calls textChanged()
   */
  ...
  /**
   * Exists due to DocumentListener interface. Detects when something is removed in the editor.
   *
   * @calls textChanged()
   */
  ...
  /**
   * Exists due to CaretListener interface. Detects when cursor position is changed.
   *
   * @calls EventManager.sendEvent()
    ...
    switch (eventIdentifier) {
      ...
      case EDITOR_MOVE_TO_LINE:
        try {
          ...
          int offset =
              this.textArea.getLineStartOffset(
                  java.lang.Math.max(((Integer) obj).intValue() - 1, 0));
  ...
  /**
   * Clear editor and start with new document. If current document has been modified since last
   * saved, then an option to save the current document is offered to the user to prevent
   * unintential loss of data.
   *
   * @calledby update()
   * @calls EventManager.sendEvent(), hasChanged(), showSaveChangesDialog(), saveDocument()
   */
  private void newDocument() {
    ...
    eventManager.sendEvent(
        EventManager.EVENT.APPLICATION_TAB_CHANGE, Integer.valueOf(GUIManager.TAB_EDITOR));

    sendCurrentDirectoryEvent();
  }

  /**
   * Show open file dialog and triggers file open event if file is successful selected. If current
   * document has been modified since last saved, then an option to save the current document is
   * offered to the user to prevent unintential loss of data.
   *
   * @pre Instance of FileChooser fc is created.
   * @calledby update()
   * @calls EventManager.sendEvent(), Editor.read(), hasChanged(), showSaveChangesDialog(),
   *     saveDocument()
   */
  ...
  /**
   * Show save file dialog and triggers file save event if file is successful selected.
   *
   * @pre Instance of FileChooser fc is created.
   ...
   *
   * @checks If document has not been associated with a file (it has not been saved before) then the
   *     Save As dialog is used instead.
   * @calledby update(), exitApplication()
  ...
  /**
   * Prepare source code and send it as event object when triggering compilation.
   *
   * @calledby update()
   * @calls EventManager.sendEvent()
   */
   ...
   * @checks If current document has been modified since last save the user is given the option save
   *     it before exiting, cancel the exit operation or exit without saving.
   * @calledby update()
  ...
  /**
   * Show dialog window offering user to save changes of current document.
   *
  ...
  /**
   * Returns document title based on filename (or untitled if document has not been saved.
   *
  ...
  /**
   * Returns document title with an asterisk appended if it has been modified.
   *
   * @return document title
   */
  ...
  private void insertInstruction(String instruction) {
    ...
    try {
      ...
      if (argument.length() > 0) newCaretPos++;
  ...
  /**
   * Implemented due to UndoableEditListener. Called whenever there is something that might be
   * undoed in the editor.
   *
  ...
  /**
   * Sends event to update the state of the undo/redo menu items.
   *
   * @calls EventManager.sendEvent()
   */
  private void updateUndoRedoState() {
    eventManager.sendEvent(
        EventManager.EVENT.EDITOR_UPDATE_UNDO_STATE, Boolean.valueOf(this.undo.canUndo()));
    eventManager.sendEvent(
        EventManager.EVENT.EDITOR_UPDATE_REDO_STATE, Boolean.valueOf(this.undo.canRedo()));
  }
  ...
  private String getIndentString(int position) {
    ...
    try {
      ...
      lineStr =
          textArea
              .getText()
              .substring(textArea.getLineStartOffset(line), textArea.getLineEndOffset(line) - 1);
  ...
  /**
   * Send an event with the current directory. If a file is open, then it is the directory of that
   * file, if no file is open null is sent.
   */
